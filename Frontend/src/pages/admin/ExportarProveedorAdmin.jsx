...
import './ImportacionAdmin.css'; // Reutiliza el mismo CSS base

const API_URL = process.env.REACT_APP_API_URL || 'http://localhost:1337';

// ─── Componente principal ──────────────────────────────────────────────────────
export default function ExportarProveedorAdmin() {
  // Auth state
  ...
  // Proveedores
  const [proveedores, setProveedores]           = useState([]);       // lista completa
  const [seleccionados, setSeleccionados]       = useState(new Set()); // seleccionados
  ...
  const cargarProveedores = useCallback(async (jwt) => {
    ...
    setErrorProveedores('');
    try {
      const res = await axios.get(`${API_URL}/api/exportar-proveedor-admin/proveedores`, {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await axios.post(`${API_URL}/api/exportar-proveedor-admin/login`, {
        identifier: email,
        password,
      });
  ...
  const seleccionarTodos = () => {
    setSeleccionados(new Set(proveedores));
    setExportOk(null);
  };
  ...
  const handleExportar = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${API_URL}/api/exportar-proveedor-admin/exportar?${queryParam}`,
        {
      ...
      <div className="ia-wrapper">
        ...
        <div className="ia-login-card">
          <div className="ia-login-brand">
            ...
            <p className="ia-brand-sub">Exportación por Proveedor</p>
          </div>
          ...
          <form className="ia-login-form" onSubmit={handleLogin} noValidate>
            ...
            <button
              id="btn-login-exportar-proveedor"
              type="submit"
  ...
  // ─── Render: Panel de Exportación por Proveedor ───────────────────────────────
  return (
    <div className="ia-wrapper">
      <div className="ia-bg-particles">
        {[...Array(12)].map((_, i) => (
      ...
      <div className="ia-panel" style={{ maxWidth: '760px' }}>
        ...
        <header className="ia-panel-header">
          <div className="ia-panel-brand">
            ...
            <div>
              <h1 className="ia-panel-title">Exportación por Proveedor</h1>
              <p className="ia-panel-sub">Marybe · Exportar productos filtrados por proveedor</p>
            </div>
          </div>
          <button
            id="btn-logout-exportar-proveedor"
        ...
        <div className="ia-section">
          <div className="ia-section-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
            ...
            {proveedores.length > 0 && (
              <div style={{ display: 'flex', gap: '8px' }}>
                <button
                  ...
                  disabled={seleccionados.size === proveedores.length}
                >
                  Todos
                </button>
                <button
          ...
          {/* Estado de carga */}
          {cargandoProveedores && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '16px 0', color: 'var(--ia-text-muted, #6b7280)' }}>
          ...
          {!cargandoProveedores && proveedores.length > 0 && (
            <div
              ...
              style={{
                ...
                overflowY:     'auto',
                padding:       '12px',
                background:    'var(--ia-bg-card, #f9fafb)',
                borderRadius:  '10px',
                ...
                return (
                  <label
                    ...
                    style={{
                      ...
                      borderRadius: '8px',
                      cursor:       'pointer',
                      background:   checked ? 'var(--ia-accent-light, #ede9fe)' : 'transparent',
                      border:       `1.5px solid ${checked ? 'var(--ia-accent, #7c6af7)' : 'transparent'}`,
                  ...
                  >
                    <input
                      ...
                      style={{ accentColor: 'var(--ia-accent, #7c6af7)', width: '16px', height: '16px', cursor: 'pointer' }}
                    />
                    <span style={{
                      fontSize:   '13px',
                      fontWeight: checked ? 600 : 400,
                      color:      checked ? 'var(--ia-accent-dark, #4c1d95)' : '#1e1b4b',
                      overflow:   'hidden',
          ...
          {!cargandoProveedores && proveedores.length === 0 && !errorProveedores && (
            <p style={{ color: 'var(--ia-text-muted, #6b7280)', fontSize: '14px', padding: '12px 0' }}>
        ...
        {exportOk && (
          <div className="ia-resultado" role="status">
            ...
            <p className="ia-resultado-time">
              Archivo: <strong>{exportOk.filename}</strong>
            </p>
            ...
            <p className="ia-resultado-time" style={{ marginTop: '4px', fontSize: '12px', opacity: 0.7 }}>
              El archivo muestra productos con sus variantes indentadas abajo, con Stock, Precio y Precio Oferta por fila.
            </p>
          </div>
        )}
